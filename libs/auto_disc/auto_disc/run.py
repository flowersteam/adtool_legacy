...
def create(parameters: Dict, experiment_id: int, seed: int,
    ...
    # insane hack guard TODO: ask Clément
    if parameters['experiment'].get('save_frequency') is not None:
        save_frequency = parameters['experiment']['save_frequency']
    elif parameters['experiment']['config'].get('save_frequency', None) is not None:
        save_frequency = parameters['experiment']['config']['save_frequency']
    else:
        raise Exception("Error loading save_frequency. Tell Jesse.")

    # Get logger
    handlers = []
    for logger_handler in parameters['logger_handlers']:
        # handler_class = get_cls_from_path(logger_handler['name'])
        handler_class = REGISTRATION['logger_handlers'][logger_handler['name']]
    ...
    explorer_factory_class = REGISTRATION['explorers'][parameters['explorer']['name']]
