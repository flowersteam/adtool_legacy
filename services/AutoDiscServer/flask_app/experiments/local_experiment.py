...
# region callbacks
    ...
    def on_save(self, **kwargs):
        ...
        super().on_save(kwargs["seed"],
                        self._get_current_checkpoint_id(kwargs["seed"]))
        self.threading_lock.release()

    def on_error(self, **kwargs):
        self.threading_lock.acquire()
        super().on_error(kwargs["seed"],
                         self._get_current_checkpoint_id(kwargs["seed"]))
        self.threading_lock.release()
...
# region saving
    def save_discovery_to_expe_db(self, **kwargs):
        ...
        to_save_outputs = copy(
            self.cleared_config["callbacks"]["on_discovery"][0]["config"]["to_save_outputs"])
        ...
        self._expe_db_caller("/discoveries/" + discovery_id +
                             "/files", files=files_to_save)

    def save_modules_to_expe_db(self, **kwargs):
        # TODO: currently only save input_wrappers, which has the new Leaf architecture
        # ideally, the experiment calls save_leaf() on *itself*
        return
        input_wrapper = kwargs["input_wrappers"][0]
        assert isinstance(input_wrapper, Leaf)
        input_wrapper.save_leaf()
        return
        ###### LA FONCTION EN JEU ######
...
# region utils
    def clean_after_experiment(self, **kwargs):
        ...
        ad_logger.handlers = [
            handler for handler in ad_logger.handlers if handler.experiment_id != self.id]
# endregion
