...
def reconstruct_parameters(experiment_id, app_db_caller):
    ...
    # Experiment
    parameters['experiment'] = _get_module_parameters(app_db_caller, 'experiments', f'id=eq.{experiment_id}')[0]

    # System
    parameters['system'] = _get_module_parameters(app_db_caller, 'systems', f'experiment_id=eq.{experiment_id}')[0]

    # Explorer
    parameters['explorer'] = _get_module_parameters(app_db_caller, 'explorers', f'experiment_id=eq.{experiment_id}')[0]

    # Input Wrappers
    parameters['input_wrappers'] = []
    parameters['input_wrappers'].extend(_get_module_parameters(app_db_caller, 'input_wrappers', f'experiment_id=eq.{experiment_id}'))
