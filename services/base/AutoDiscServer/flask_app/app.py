...
from auto_disc.utils.leafutils.leafstructs.registration import (
    get_auto_disc_registered_callbacks,
    get_auto_disc_registered_modules_info,
    get_cls_from_path,
    get_modules,
)
...
def list_output_representations():
    info = get_auto_disc_registered_modules_info(get_modules("output_representations"))
    return make_response(jsonify(info), 200)
...
def list_input_wrappers():
    info = get_auto_disc_registered_modules_info(get_modules("input_wrappers"))
    return make_response(jsonify(info), 200)
