...
class RemoteExperiment(BaseExperiment):
    ...
    def __init__(self, host_profile_name, *args, **kwargs):
        self.__host_profile = parse_profile(
            next(
                profile[1]
                for profile in list_profiles()
                if profile[0] == host_profile_name
            )
        )

        args[1]["logger_handlers"] = [
            {
                "name": "logFile",
                "config": {
            ...
            "on_discovery": [
                {
                    "name": "disk",
                    "config": {
                        "to_save_outputs": args[1]["experiment"]["config"][
            ...
            "on_saved": [
                {
                    "name": "disk",
                    "config": {
                        "folder_path": self.__host_profile["work_path"]
                        + "/checkpoints/"
                    },
                }
            ],
            "interact": [
                {
                    "name": "saveDisk",
                    "config": {
                        "folder_path": self.__host_profile["work_path"] + "/data_saves/"
                    },
                },
                {
                    "name": "readDisk",
    ...
    def start(self):
        ...
        exec_command = self.__host_profile["execution"].replace(
            "$NB_SEEDS",
            "{}".format(self.experiment_config["experiment"]["config"]["nb_seeds"]),
        )
        exec_command = exec_command.replace(
            "$ARGS",
            "--config_file {}/parameters_remote.json --experiment_id {} --nb_iterations {}".format(
    ...
    def reload(self):
        response = self._app_db_caller(
            "/experiments?id=eq.{}".format(self.id), AppDBMethods.GET, {}
        )
    ...
    def stop(self):
        ...
        self.nb_seeds_finished = self.experiment_config["experiment"]["config"][
            "nb_seeds"
        ]
        ...
        self.callback_to_all_running_seeds(
            lambda seed, current_checkpoint_id: self.on_cancelled(seed)
        )
    ...
    def _send_packaged_experiment_to_remote(self):
        # where we saved remote files need to be pushed on server
        to_push_folder_path = (
            self.__host_profile["local_tmp_path"] + "/remote_experiment/push_to_server"
        )
        ...
        # make path
        lib_path = (
            os.path.dirname(os.path.realpath(__file__)) + "/../../../../libs/auto_disc"
        )
        ...
        self.__untar_folder(
            self.__host_profile["work_path"] + "/libs.tar.gz",
            self.__host_profile["work_path"],
        )
        ...
        additional_file_path = (
            ...
            + "/../../../../configs/remote_experiments/additional_files"
        )
        ...
        self.__untar_folder(
            self.__host_profile["work_path"] + "/additional_files.tar.gz",
            self.__host_profile["work_path"],
        )
        ...
        self.shell.sendline(
            "mkdir {}/{}".format(self.__host_profile["work_path"], "logs")
        )
        self.shell.sendline(
            "mkdir {}/{}".format(self.__host_profile["work_path"], "run_ids")
        )
        self.shell.sendline(
            "mkdir {}/{}".format(self.__host_profile["work_path"], "out")
        )
        self.shell.sendline(
    ...
    def __tar_local_folder(self, folder_src, tar_path):
        ...
        os.system(
            "cd {} && tar -czvf {} {}".format(parent_folder, tar_path, target_folder)
        )
    ...
    def __pull_files(self, remote_path, sub_folders, run_idx, local_folder):
        ...
        for sub_folder in sub_folders:
            ...
            self.__pull_folder(
                "{}{}/idx_{}.*".format(remote_path, sub_folder, run_idx),
                "{}{}/".format(local_folder, sub_folder),
            )
    ...
    def send_feedback_to_remote_server(self, request_dict):
        ...
        local_path = self.__host_profile[
            "local_tmp_path"
        ] + "/remote_experiment/push_to_server/data_saves/{}/{}".format(
    ...
    def _monitor(self):
        ...
        # make path
        # where we saved remote experiments output before puting them in our db
        local_folder = self.__host_profile["local_tmp_path"] + "/remote_experiment/out"
        ...
        while not self.test_file_exist(
            ...
            self._app_db_caller(
                "/preparing_logs",
                AppDBMethods.POST,
                {"experiment_id": self.id, "message": "waiting for the server"},
            )
            sleep(self.__host_profile["check_experiment_launched_every"])
        self.shell.sendline(
            "tail -F -n +1 {}".format(
        ...
        response = self._app_db_caller(
            "/experiments?id=eq.{}".format(self.id),
            AppDBMethods.PATCH,
        ...
        self._app_db_caller(
            "/preparing_logs",
            AppDBMethods.POST,
            {"experiment_id": self.id, "message": "the experiment start"},
        )
    ...
    def __parse_log(self, log, local_folder):
        ...
        logger_name, log_level_name, seed_number, log_id, message = self.__get_log_part(
            log
        )
            ...
            while (
                ...
                if "ad_tool_logger -" in current_log_line:
                    ...
                    (
                        ...
                        log_id,
                        message,
                    ) = self.__parse_log(copy(current_log_line), local_folder)

                    if self.__log_not_in_db(log_id):
                        previous_checkpoint_ids.append(
                            self._get_current_checkpoint_id(seed_number)
                        )
                        if previous_log is not None and previous_message is not None:
                            self.app_db_logger_handler.save(
                                ...
                                self.app_db_logger_handler.log_levels_id[
                                    previous_log_level_name
                                ],
                                log_id,
                                previous_message,
                            )
    ...
    def __log_not_in_db(self, log_name):
        response = self._app_db_caller(
            "/logs?&name=eq.{}".format(log_name), AppDBMethods.GET, None
        )
        return response.content.decode() == "[]"

    def __new_files_saved_on_remote_disk(self, log):
        return (
            "New discovery saved" in log
            or "New modules saved" in log
            or "New data saved" in log
        )
    ...
    def save_modules_to_expe_db(self, **kwargs):
        try:
            ...
            self._expe_db_caller(
                "/checkpoint_saves/" + module_id + "/files", files=files_to_save
            )
        except Exception as ex:
            print(
        ...
        try:
            ...
            for module in to_save:
                if module == "dict_info":
                    ...
                    files_to_save[module] = open(
                        folder + module + "/idx_{}.pickle".format(kwargs["run_idx"]),
                        "rb",
                    )
            request_dict = {
                "experiment_id": self.id,
            ...
            data_id = self._expe_db_caller("/data_saves", request_dict=request_dict)[
                "ID"
            ]
            self._expe_db_caller(
                "/data_saves/" + data_id + "/files", files=files_to_save
            )
