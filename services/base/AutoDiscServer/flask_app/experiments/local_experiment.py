...
class LocalExperiment(BaseExperiment):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._additional_callbacks = {
            "on_discovery": [self.on_progress, SaveDiscoveryInExpeDB()],
    ...
    def prepare(self):
        self._app_db_caller(
            "/preparing_logs",
            AppDBMethods.POST,
            {"experiment_id": self.id, "message": "experimence in preparation"},
        )
    ...
    def start(self):
        print(
            "Starting local experiment with id {} and {} seeds".format(
    ...
    def stop(self):
        print(
            "Stopping {} seeds of local experiment with id {}".format(
    ...
    def reload(self):
        self._app_db_caller(
            "/checkpoints?experiment_id=eq.{}&status=eq.{}".format(
                self.id, int(CheckpointsStatusEnum.RUNNING)
            ),
            AppDBMethods.PATCH,
    ...
    def on_error(self, **kwargs):
        self.threading_lock.acquire()
        super().on_error(
            kwargs["seed"], self._get_current_checkpoint_id(kwargs["seed"])
        )
        self.threading_lock.release()
    ...
    def clean_after_experiment(self, **kwargs):
        ...
        ad_logger.handlers = [
            ...
            if handler.experiment_id != self.id
        ]
