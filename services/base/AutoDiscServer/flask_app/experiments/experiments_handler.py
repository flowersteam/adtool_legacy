...
class ExperimentsHandler:
    ...
    def _reload_experiment(self, id, parameters):
        try:
            ...
            message = "Error when reloading experiment: {}".format(
    ...
    def reload_running_remote_experiments(self):
        ...
        response = self._app_db_caller(
            "/experiments?exp_status=eq.{}".format(ExperimentStatusEnum.RUNNING),
            AppDBMethods.GET,
            {},
        )
    ...
    def add_experiment(self, parameters):
        try:
            ...
            r = self._app_db_caller("/experiments", AppDBMethods.POST, request)
            ...
            try:
                ...
                self._app_db_caller(
                    "/preparing_logs",
                    AppDBMethods.POST,
                    {"experiment_id": id, "message": message},
                )
                raise Exception(message)
    ...
    def on_progress_callback(self, experiment_id, progress):
        self._app_db_caller(
            "/experiments?id=eq.{}".format(experiment_id),
            AppDBMethods.PATCH,
            {"progress": progress},
        )
