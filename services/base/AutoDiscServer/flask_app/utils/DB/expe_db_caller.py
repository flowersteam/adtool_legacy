...
class ExpeDBCaller:
    ...
    def __call__(
        self,
        route: str,
        request_dict: typing.Dict[str, Any] = None,
        files: typing.Dict[str, Any] = None,
    ) -> Dict:
    ...
    def read(self, route: str, filter_attribut: dict):
        ...
        response = json.loads(
            requests.get(self.base_url + route + "?filter=" + filter).content
        )
        return response
        ...
        if isinstance(response, list):
            for i in range(0, len(response)):
                response[i]["file_" + file_name] = requests.get(
                    self.base_url + route + "/" + response[i]["_id"] + "/" + file_name
                ).content
                try:
                    response[i]["file_" + file_name] = pickle.loads(
            ...
            response["file_" + file_name] = requests.get(
                self.base_url + route + "/" + response["_id"] + "/" + file_name
            ).content
            try:
                response["file_" + file_name] = pickle.loads(
                    response["file_" + file_name]
                )
            except:
                pass
        return response
