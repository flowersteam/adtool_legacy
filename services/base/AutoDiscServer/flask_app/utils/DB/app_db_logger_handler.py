...
class AppDBLoggerHandler(StreamHandler):
    ...
    def emit(self, record: logging.LogRecord):
        if self.experiment_id == record.args["experiment_id"]:
            ...
            checkpoint_id = self.get_checkpoint_id_from_seed_fn(
                record.args["seed"])
            self.save(
                checkpoint_id, record.args["seed"], levelname, record.args["id"], record.msg)

    def save(self, checkpoint_id: int, seed: int, level_name: int, log_id: str, message: str):
