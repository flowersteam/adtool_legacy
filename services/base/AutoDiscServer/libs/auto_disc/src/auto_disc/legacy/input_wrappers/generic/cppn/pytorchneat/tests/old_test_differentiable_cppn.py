...
class TestRecurrentNetwork(TestCase):
    def test_pytorchneat_differentiability(self):
                ...
                for j in range(img_width):
                    if torch.rand(()) < 0.7:
                        coords.append(torch.tensor(
                            [i, j], dtype=torch.float64))
                        feats.append(torch.tensor(
                            [i, j, cppn_input[i, j, 2], 1]))
            cppn_input = ME.SparseTensor(
                torch.stack(feats), torch.stack(coords))
        mnist_dataset = torchvision.datasets.MNIST(
            root="/home/mayalen/data/pytorch_datasets/mnist/", download=False, train=True)
        upscale_target_tansform = transforms.Compose([transforms.ToPILImage(
        ...
        for mnist_target_idx in range(1, 21):
            ...
            target_img = upscale_target_tansform(
                mnist_dataset.data[mnist_target_idx]).squeeze().float()
                ...
                for i, row in enumerate(target_img):
                    for j, val in enumerate(row):
                        if val != 0:
                            coords.append([i, j])
                            feats.append([val])
                target_img = ME.SparseTensor(features=torch.DoubleTensor(feats), coordinates=torch.IntTensor(
                    coords), coordinate_manager=cppn_input.coordinate_manager)
                ...
                for genome_idx, genome in genomes:
                    ...
                    for train_step in range(45):
                        ...
                        if isinstance(cppn_net_output, torch.Tensor):
                            cppn_net_output = (
                                1.0 - cppn_net_output.abs()).view(img_height, img_width)
                        ...
                        if isinstance(cppn_net_output, torch.Tensor):
                            ...
                            pcd.points = o3d.utility.Vector3dVector(torch.cat(
                                [cppn_net_output.C, torch.zeros([cppn_net_output.C.shape[0], 1])], -1).cpu().detach())
                            ...
                            train_images.append(torch.from_numpy(
                                np.asarray(out_image)).transpose(0, -1))
                    t1 = time.time()
                    ...
                    plt.imshow(torchvision.utils.make_grid(
                        train_images, 15).permute(1, 2, 0))
                    plt.tight_layout()
                    plt.savefig(os.path.join(cur_output_dir,
                                f'individual_{genome.key}.png'))
                    plt.close()
