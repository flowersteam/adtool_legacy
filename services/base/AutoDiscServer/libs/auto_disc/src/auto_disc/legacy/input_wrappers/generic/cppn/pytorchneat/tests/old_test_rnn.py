...
class TestRecurrentNetwork(TestCase):
    ...
    def test_pytorchneat_vs_pythonneat(self):
        ...
        for net_idx in range(num_networks):
            ...
            pytorch_cppn_output_1 = (
                1.0 - pytorch_cppn_output.abs()).cpu().detach()
            t1 = time.time()
            ...
            pytorch_cppn_output = pytorch_cppn.activate(
                cppn_input, recurrent_net_passes)
            pytorch_cppn_output_4 = (
                1.0 - pytorch_cppn_output.abs()).cpu().detach()
            t1 = time.time()
            print(
                f"Pytorch distributed RNN GPU calculation took {t1 - t0} secs")
            ...
            neat_cppn_output = (1.0 - neat_cppn_output.abs()
                                ).cpu().detach().view(img_height, img_width, 1)
            t1 = time.time()
        ...
        for net_idx in range(num_networks):
            ...
            t1 = time.time()
            print(
                f"Pytorch vectorized RNN CPU calculation took {t1 - t0} secs")
            ...
            t1 = time.time()
            print(
                f"Pytorch vectorized RNN GPU calculation took {t1 - t0} secs")
            ...
            t1 = time.time()
            print(
                f"Pytorch distributed RNN CPU calculation took {t1 - t0} secs")
