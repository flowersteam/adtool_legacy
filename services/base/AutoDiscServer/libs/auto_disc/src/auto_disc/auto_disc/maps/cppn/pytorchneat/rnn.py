import graphviz
import copy
from auto_disc.legacy.input_wrappers.generic.cppn.pytorchneat.aggregations import str_to_aggregation
from auto_disc.legacy.input_wrappers.generic.cppn.pytorchneat.activations import str_to_activation
...
class RecurrentNetwork(nn.Module):
    ...
    def __init__(self, input_neuron_ids, hidden_neuron_ids, output_neuron_ids, biases, responses, activations, connections, device='cpu'):
        ...
        self.hidden_key_to_idx = {k: i for i,
                                  k in enumerate(hidden_neuron_ids)}
        self.n_outputs = len(output_neuron_ids)
        self.output_key_to_idx = {k: i for i,
                                  k in enumerate(output_neuron_ids)}
        ...
        for k, v in connections.items():
            if k[1] in hidden_neuron_ids:
                if k[0] in input_neuron_ids:
                    input_to_hidden[self.input_key_to_idx[k[0]],
                                    self.hidden_key_to_idx[k[1]]] = v
                    input_to_hidden_mask[self.input_key_to_idx[k[0]],
                                         self.hidden_key_to_idx[k[1]]] = True
                elif k[0] in hidden_neuron_ids:
                    hidden_to_hidden[self.hidden_key_to_idx[k[0]],
                                     self.hidden_key_to_idx[k[1]]] = v
                    hidden_to_hidden_mask[self.hidden_key_to_idx[k[0]],
                                          self.hidden_key_to_idx[k[1]]] = True
                elif k[0] in output_neuron_ids:
                    output_to_hidden[self.output_key_to_idx[k[0]],
                                     self.hidden_key_to_idx[k[1]]] = v
                    output_to_hidden_mask[self.output_key_to_idx[k[0]],
                                          self.hidden_key_to_idx[k[1]]] = True
            elif k[1] in output_neuron_ids:
                if k[0] in input_neuron_ids:
                    input_to_output[self.input_key_to_idx[k[0]],
                                    self.output_key_to_idx[k[1]]] = v
                    input_to_output_mask[self.input_key_to_idx[k[0]],
                                         self.output_key_to_idx[k[1]]] = True
                elif k[0] in hidden_neuron_ids:
                    hidden_to_output[self.hidden_key_to_idx[k[0]],
                                     self.output_key_to_idx[k[1]]] = v
                    hidden_to_output_mask[self.hidden_key_to_idx[k[0]],
                                          self.output_key_to_idx[k[1]]] = True
                elif k[0] in output_neuron_ids:
                    output_to_output[self.output_key_to_idx[k[0]],
                                     self.output_key_to_idx[k[1]]] = v
                    output_to_output_mask[self.output_key_to_idx[k[0]],
                                          self.output_key_to_idx[k[1]]] = True
        self.register_parameter(
            'input_to_hidden', nn.Parameter(input_to_hidden))
        self.register_buffer('input_to_hidden_mask', input_to_hidden_mask)
        self.register_parameter(
            'hidden_to_hidden', nn.Parameter(hidden_to_hidden))
        self.register_buffer('hidden_to_hidden_mask', hidden_to_hidden_mask)
        self.register_parameter(
            'output_to_hidden', nn.Parameter(output_to_hidden))
        self.register_buffer('output_to_hidden_mask', output_to_hidden_mask)
        self.register_parameter(
            'input_to_output', nn.Parameter(input_to_output))
        self.register_buffer('input_to_output_mask', input_to_output_mask)
        self.register_parameter(
            'hidden_to_output', nn.Parameter(hidden_to_output))
        self.register_buffer('hidden_to_output_mask', hidden_to_output_mask)
        self.register_parameter(
            'output_to_output', nn.Parameter(output_to_output))
        ...
        hidden_mask = (self.input_to_hidden_mask.sum(
            0)+self.hidden_to_hidden_mask.sum(0)+self.output_to_hidden_mask.sum(0)) > 0
        self.register_buffer('hidden_mask', hidden_mask)
        output_mask = (self.input_to_output_mask.sum(
            0) + self.hidden_to_output_mask.sum(0) + self.output_to_output_mask.sum(0)) > 0
        ...
        self.hidden_responses = nn.Parameter(
            torch.tensor(self.hidden_responses))
        self.output_responses = nn.Parameter(
            torch.tensor(self.output_responses))
    ...
    def forward(self, inputs):
        ...
        after_pass_hidden_activs = torch.zeros_like(
            self.hidden_activs.detach()).to(self.device)
        after_pass_output_activs = torch.zeros_like(
            self.output_activs.detach()).to(self.device)
        ...
        after_pass_hidden_activs = inputs.mm(self.input_to_hidden) + \
            ...
            self.output_activs.mm(
                self.output_to_hidden)  # batch_size, n_hidden

        after_pass_output_activs = inputs.mm(self.input_to_output) + \
            self.hidden_activs.mm(self.hidden_to_output) + \
            self.output_activs.mm(
                self.output_to_output)  # batch_size, n_outputs

        # Step 2: x = agg(x)
        # here sum aggregation already happened in Step 1 within torch.mm

        # Step 3: x = bias + reponse * x
        after_pass_hidden_activs = self.hidden_biases + \
            self.hidden_responses * after_pass_hidden_activs
        after_pass_output_activs = self.output_biases + \
            self.output_responses * after_pass_output_activs

        # Step 3: x = act(x)
        for hidden_neuron_idx in range(self.n_hidden):
            after_pass_hidden_activs[:, hidden_neuron_idx] = self.hidden_activations[hidden_neuron_idx](
                after_pass_hidden_activs[:, hidden_neuron_idx].clone())
        for output_neuron_idx in range(self.n_outputs):
            after_pass_output_activs[:, output_neuron_idx] = self.output_activations[output_neuron_idx](
    ...
    def activate(self, inputs, n_passes=1):
        ...
        self.hidden_activs = torch.zeros(
            (batch_size, self.n_hidden)).to(self.device)
        self.output_activs = torch.zeros(
            (batch_size, self.n_outputs)).to(self.device)
    ...
    def draw_net(self, view=False, filename=None, node_names=None, node_colors=None, fmt='svg'):
        ...
        for from_k, to_k in torch.cat([torch.stack(torch.where(self.input_to_hidden)).t(),
                                       torch.stack(torch.where(
                                           self.hidden_to_hidden)).t(),
                                       torch.stack(torch.where(
                                           self.output_to_output)).t(),
                                       torch.stack(torch.where(
                                           self.input_to_output)).t(),
                                       torch.stack(torch.where(
                                           self.hidden_to_output)).t(),
                                       torch.stack(torch.where(self.output_to_output)).t()]):
            connections[(from_k.item(), to_k.item())
                        ] = self.input_to_hidden[from_k, to_k]
        for cg, cg_weight in connections.items():
            ...
            dot.edge(a, b, _attributes={'style': style, 'color': color, 'fontcolor': color,
                     'penwidth': width, 'fontsize': '9', 'label': f'w={cg_weight:.1f}'})
...
class RecurrentNetworkAgg(nn.Module):
    ...
    def __init__(self, input_neuron_ids, hidden_neuron_ids, output_neuron_ids, biases, responses, activations, aggregations, connections, device='cpu'):
        ...
        for k, v in connections.items():
            connections_with_str_keys[tuple_to_str_key(
    ...
    def forward(self, inputs):
        ...
        if inputs.device != self.device:
            inputs = inputs.to(self.device)
        after_pass_node_activs = torch.zeros_like(
            self.node_activs).to(self.device)

        for neuron_key in self.output_neuron_ids+self.hidden_neuron_ids:
            incoming_connections = [c for c in self.connections.keys() if str_to_tuple_key(c)[
                1] == neuron_key]
            ...
            for conn in incoming_connections:
                ...
                if input_key < 0:
                    incoming_inputs = torch.vstack(
                        [incoming_inputs, self.connections[conn] * inputs[:, self.input_key2idx[input_key]]])
                # neuron_to_neuron case
                else:
                    incoming_inputs = torch.vstack(
                        [incoming_inputs, self.connections[conn] * self.node_activs[:, self.neuron_key2idx[input_key]]])

            # aggregate incoming inputs
            if len(incoming_connections) > 0:
                node_outputs = self.activation_functions[str(neuron_key)](self.biases[str(
                    neuron_key)] + self.responses[str(neuron_key)] * self.aggregation_functions[str(neuron_key)](incoming_inputs.t()))
            else:
                node_outputs = torch.zeros((batch_size))
            after_pass_node_activs[:,
                                   self.neuron_key2idx[neuron_key]] = node_outputs
    ...
    def activate(self, inputs, n_passes=1):
        ...
        self.node_activs = torch.zeros(
            (batch_size, self.n_outputs+self.n_hidden)).to(self.device)
        for _ in range(n_passes):
