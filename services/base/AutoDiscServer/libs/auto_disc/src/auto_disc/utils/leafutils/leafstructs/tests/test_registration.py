...
def test_get_cls_from_path():
    ...
    cls = get_cls_from_path(path)
    from adtool_default.maps.LeniaStatistics import \
...
def test_get_legacy_modules():
    ...
    assert get_legacy_modules("callbacks").keys() == \
        set(["on_discovery",
             "on_cancelled",
             "on_error",
             "on_finished",
...
def test_get_default_modules():
    ...
    assert get_default_modules("maps") == {
        "MeanBehaviorMap":
        "adtool_default.maps.MeanBehaviorMap.MeanBehaviorMap",
        "UniformParameterMap":
        "adtool_default.maps.UniformParameterMap.UniformParameterMap",
        "LeniaStatistics":
        "adtool_default.maps.LeniaStatistics.LeniaStatistics",
        "UniformParameterMap":
        "adtool_default.maps.UniformParameterMap.UniformParameterMap",
        "LeniaParameterMap":
        "adtool_default.maps.LeniaParameterMap.LeniaParameterMap",
        "NEATParameterMap":
        "adtool_default.maps.NEATParameterMap.NEATParameterMap"
    }
...
def test_get_modules():
    assert set(get_modules("systems").keys()) == \
        set(["ExponentialMixture", "Lenia",
             "LeniaCPPN"])
    assert set(get_modules("explorers").keys()) == \
        set(["IMGEPExplorer"])
    assert set(get_modules("maps").keys()) == \
        set(["MeanBehaviorMap", "UniformParameterMap", "LeniaStatistics",
             "LeniaParameterMap", "NEATParameterMap"
             ])
    assert set(
        get_modules("callbacks").keys()) == set(
        [
            "on_discovery",
            "on_cancelled",
            "on_error",
