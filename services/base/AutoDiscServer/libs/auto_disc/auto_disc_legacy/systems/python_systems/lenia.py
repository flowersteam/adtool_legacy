...
from auto_disc_legacy.systems.python_systems import BasePythonSystem
...
from auto_disc_legacy.utils.mutators import GaussianMutator
...
class Lenia(BasePythonSystem):
    ...
    input_space = DictSpace(
        ...
        R=DiscreteSpace(n=20, mutator=GaussianMutator(
            mean=0.0, std=0.5), indpb=1.0),
        T=BoxSpace(low=1.0, high=20.0, mutator=GaussianMutator(
            mean=0.0, std=0.5), shape=(), indpb=1.0, dtype=torch.float32),
        b=BoxSpace(low=0.0, high=1.0, mutator=GaussianMutator(
            mean=0.0, std=0.1), shape=(4,), indpb=1.0, dtype=torch.float32),
        m=BoxSpace(low=0.0, high=1.0, mutator=GaussianMutator(
            mean=0.0, std=0.1), shape=(), indpb=1.0, dtype=torch.float32),
        s=BoxSpace(low=0.001, high=0.3, mutator=GaussianMutator(
            mean=0.0, std=0.05), shape=(), indpb=1.0, dtype=torch.float32),
    ...
    output_space = DictSpace(
        states=BoxSpace(low=0, high=1, shape=(ConfigParameterBinding(
            "final_step"), ConfigParameterBinding("SX"), ConfigParameterBinding("SY")))
    )
    ...
    def reset(self, run_parameters: typing.Dict[str, torch.Tensor]) -> typing.Dict[str, torch.Tensor]:
        run_parameters.kn = 0
        run_parameters.gn = 1
        init_state = torch.zeros(1, 1, self.config.SY,
                                 self.config.SX, dtype=torch.float64)
        init_state[
            0,
            0,
    ...
    def step(self, action=None) -> typing.Tuple[typing.Dict[str, torch.Tensor], int, bool, None]:
        if self._step_idx >= self.config.final_step:
            raise Exception(
                "Final step already reached, please reset the system.")
