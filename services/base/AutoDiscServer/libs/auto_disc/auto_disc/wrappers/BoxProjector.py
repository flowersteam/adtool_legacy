...
class BoxProjector(Leaf):
    ...
    def _clamp_and_truncate(self, data: torch.Tensor) -> torch.Tensor:
        if self.bound_lower == torch.tensor(
            [float("-inf")]
        ) and self.bound_upper == torch.tensor([float("inf")]):
            # short circuit to avoid Python initialization problems with tensors on different devices
            return data
        else:
