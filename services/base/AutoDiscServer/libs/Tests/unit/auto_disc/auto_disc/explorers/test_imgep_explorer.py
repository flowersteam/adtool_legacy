...
auto_discFolderPath = os.path.abspath(os.path.join(
    classToTestFolderPath, "../"*6 + "/libs/auto_disc/auto_disc"))
...
def test_initialize_1():
    ...
    output_space = []
    def input_distance_fn(): return 5
...
def test_expand_goal_space_1():
    ...
    # TODO
    assert (
        torch.equal(space.high, torch.tensor(
            [12.0, 13.0], dtype=torch.float32))
...
def test__get_next_goal_2():
    ...
    assert "Unknown goal generation type 'notRandom' in the configuration!" == context.exception.args[
        0]
...
def test_get_source_policy_idx_1():
    ...
    input_space.sample = lambda: {"a": "sample has been executed"}
    output_space = []
    def input_distance_fn(x, y): return torch.tensor([0, 1, 2])
    imgepExplorer.initialize(input_space, output_space, input_distance_fn)
    history = [{"embedding": torch.zeros(5)}]
    target_goal = torch.zeros(10)
    source_policy_idx = imgepExplorer._get_source_policy_idx(
        target_goal, history)
...
def test_get_source_policy_idx_3():
    ...
    with unittest.TestCase.assertRaises(Exception, ValueError) as context:
        source_policy_idx = imgepExplorer._get_source_policy_idx(
            target_goal, history)
    assert "Unknown source policy selection type 'somethingToMakeError' in the configuration!" == context.exception.args[
        0]
# endregion
...
def test_sample_2():
    ...
    imgepExplorer._access_history = lambda: {
        "input": [{"embedding": torch.zeros(5)}], 0: {'output': "what's you need"}}
