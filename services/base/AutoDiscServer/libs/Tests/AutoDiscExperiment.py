...
if __name__ == "__main__":
    ...
    experiment = ExperimentPipeline(
        ...
        save_frequency=10,
        system=PythonLenia(
            final_step=200, scale_init_state=1.0, logger=logger),
        ...
        output_representations=[LeniaHandDefinedRepresentation(
            logger=logger)],  # Starting from the system !
        on_discovery_callbacks=[CustomPrintCallback("Newly explored output !", logger=logger),
                                OnDiscoverySaveCallbackOnDisk("./experiment_results/", logger=logger,
                                                              to_save_outputs=["raw_run_parameters",
                                                                               ...
                                                                               "output",
                                                                               "rendered_output"])],
        on_save_callbacks=[OnSaveModulesOnDiskCallback(
            "./experiment_results/", logger=logger)],
    )
