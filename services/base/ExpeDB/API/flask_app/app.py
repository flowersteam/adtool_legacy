...
CORS(app)
client = MongoClient(
    "mongodb://{}:{}/".format(config.MONGODB_HOST, config.MONGODB_PORT),
...
def _get_one_by_filter(collection, filter, query=None):
    ...
    if element:
        ...
        return make_response(
            "No element found matching {} in collection {}".format(filter, collection),
            403,
        )
...
def _get_file_from_document(document, filename, _filename: str = ""):
    """
    ...
    """
    error_msg = "No file {} found with in document with id {}".format(
...
def list_discoveries():
    ...
    if filter is not None:
        ...
        return _get_multiple_by_filter(
            db.discoveries, json.loads(filter), json.loads(query) if query else None
        )
    else:
...
def get_discovery_by_id(id):
    ...
    if query is not None:
        return _get_one_by_filter(
            db.discoveries, {"_id": ObjectId(id)}, query=json.loads(query)
        )
    else:
...
def delete_discoveries():
    ...
    if checkpoint_id is not None:
        ...
        return make_response(
            "You must provide a checkpoint_id in the request args", 403
        )
...
def add_checkpoint_save_files(id):
    ...
    checkpoint_save = db.checkpoint_saves.find_one({"_id": ObjectId(id)})
    if checkpoint_save:
        return _add_files_to_document(
            db.checkpoint_saves, checkpoint_save, request.files
        )
    else:
        return make_response(f"No checkpoint_save found with id {id}", 403)
...
def list_data_saves():
    ...
    if filter is not None:
        query = request.args.get("query", default=None)
        return _get_multiple_by_filter(
            db.data_saves, json.loads(filter), json.loads(query) if query else None
        )
    else:
        return make_response("You must provide a " "filter in the request args", 403)
