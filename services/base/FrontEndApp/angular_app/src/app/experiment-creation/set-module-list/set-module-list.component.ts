...
export class SetModuleListComponent implements OnInit {
  ...
  drop(event: CdkDragDrop<string[]>, dragdrop_array: any[]) {
    ...
    moveItemInArray(
      ...
      event.previousIndex,
      event.currentIndex
    ); //move custom config module link to a module
    let response = this.createNewExperimentService.defineCustomModuleList(
      this.currentModuleList,
      this.customModules,
      this.modules,
      this.key,
      this.spaceItDependsOn
    );
  ...
  remove(index: number) {
    let response = this.createNewExperimentService.removeModuleToUse(
      this.currentModuleList,
      this.customModules,
      this.modules,
      this.key,
