...
export class ExperimentCreationComponent implements OnInit {
  ...
  createExperiment() {
    if (this.createNewExperimentService.checkNewExperimentSet()) {
      ...
      (<HTMLInputElement>document.getElementById('btn_create_exp')).disabled =
        true;
      this.AutoDiscServerService.createExperiment(
        this.createNewExperimentService.newExperiment
      ).subscribe((res) => {
        if (!res.success) {
          (<HTMLInputElement>(
            document.getElementById('btn_create_exp')
          )).disabled = false;
          ...
          if (this.createNewExperimentService.newExperiment.experiment.name) {
            this.JupyterService.createNotebookDir(
              ...
              if (experiment_status != 4) {
                this.router.navigate([
                  '/experiment/' + experiment_id.toString(),
                ]);
              }
            });
          }
