...
export class JupyterService {
  ...
  constructor(private http: HttpClient, private appDBService: AppDbService) {
    this.jupyterHost =
      environment.GATEWAY_HOST + ':' + environment.GATEWAY_PORT;
  ...
  openKernelChannel(kernel_id: string): Observable<any> {
    this.jupyWebSocket = webSocket(
      'ws://' + this.jupyterHost + '/api/kernels/' + kernel_id + '/channels'
    );
    this.jupyWebSocket.subscribe(
      (msg: any) => {
        if (
          msg.parent_header.msg_id == this.msg_id_send &&
      ...
      (err: any) =>
        console.log('error append with jupyter kernel server: ' + err), // Called if at any point WebSocket API signals some kind of error.
