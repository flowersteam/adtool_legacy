...
export class CreateNewExperimentService {
  ...
  customConfig: any =
    []; /* some cutsom module config use for input_wrapper and output_representation. 
  ...
  stagedExperiment: BehaviorSubject<ExperimentSettings> = new BehaviorSubject(
  ...
  getOutputRepresentations(): void {
    this.AutoDiscServerService.getOutputRepresentations().subscribe(
      (response) => {
        if (response.success) {
          ...
          this.toasterService.showError(
            response.message ?? '',
            'Error getting output representations',
            { timeOut: 0, extendedTimeOut: 0 }
          );
        }
      }
  ...
  checkNewExperimentSet() {
    ...
    if (this.newExperiment.input_wrappers.length == 0) {
      this.toasterService.showInfo(
        'No Input Wrappers defined',
        'Input Wrappers'
      );
    }
    if (this.newExperiment.output_representations.length == 0) {
      this.toasterService.showInfo(
        'No output representations defined',
        'Output Representations'
      );
    }
    if (
      this.newExperiment.experiment.name == undefined ||
      this.newExperiment.experiment.name == ''
    ) {
      this.toasterService.showWarning(
        'Experiment has no name',
        'General Information'
      );
  ...
  checkExperimentName() {
    ...
    if (
      this.newExperiment.experiment.name &&
      !alphanumericRegex.test(this.newExperiment.experiment.name)
    ) {
      let spaceRegex = /(\s)/g;
      let notAlphanumericRegex = /[^a-zA-Z0-9_-]+/g;
      this.newExperiment.experiment.name =
        this.newExperiment.experiment.name?.trim();
      this.newExperiment.experiment.name =
        this.newExperiment.experiment.name?.replace(spaceRegex, '_');
      this.newExperiment.experiment.name =
        this.newExperiment.experiment.name?.replace(notAlphanumericRegex, '');
      this.newExperiment.experiment.name =
        this.newExperiment.experiment.name?.replace(/^_+|_+$/g, ''); // equal to trim("_") in other language
      this.toasterService.showWarning(
        'the name of the experiment has been changed to match alphanumeric characters',
        'Experiment name'
      );
  ...
  setModuleUse(currentModule: any, moduleName: string, modules: any) {
    ...
    let temp_module_config = CreateNewExperimentService.getModuleByName(
      modules,
      moduleName
    ).config;
  ...
  makeDisplayableInputSpace(module: any, currentModule: any) {
    ...
    if (module.config.wrapped_output_space_key != undefined) {
      ...
      let index = input_space.indexOf(
        currentModule.config.wrapped_output_space_key
      );
  ...
  makeDisplayableOutputSpace(module: any, currentModule: any) {
    ...
    if (module.config.wrapped_input_space_key != undefined) {
      ...
      let index = output_space.indexOf(
        currentModule.config.wrapped_input_space_key
      );
  ...
  makeColorToDisplayInputOutputSpace(
    module: any,
    currentModule: any,
    inputList: string[],
    outputList: string[]
  ...
  setAllCustomModulesFromUseModule() {
    this.customConfig.input_wrappers = [];
    this.customConfig.output_representations = [];
    this.setOneTypeOfCustomModulesFromUseModule(
      this.customConfig.input_wrappers,
      this.allConfig.input_wrappers,
      this.newExperiment.input_wrappers,
      'input_wrapper'
    );
    this.setOneTypeOfCustomModulesFromUseModule(
      this.customConfig.output_representations,
      this.allConfig.output_representations,
      this.newExperiment.output_representations,
      'output_representation'
    );
  }

  setOneTypeOfCustomModulesFromUseModule(
    customModules: any,
    modules: any,
    useModules: any,
    moduleType: string
    ...
    if (moduleType == 'input_wrapper') {
      ...
      spaceItDependsOn = this.getModuleByName(
        this.allConfig.systems,
  ...
  addCustomModuleToList(
    customModules: any,
    modules: any,
    moduleName: string,
    key: string | undefined,
    spaceItDependsOn: any
  ) {
    let index: number;
    modules[0].input_space != undefined
      ? (index = 0)
      : (index = customModules.length);
    customModules.splice(
      index,
      0,
      cloneDeep(CreateNewExperimentService.getModuleByName(modules, moduleName))
    );

    if (customModules[index].input_space == undefined) {
      if (customModules.length > 1) {
        spaceItDependsOn = customModules[index - 1].output_space;
      }
      customModules[index].input_space = spaceItDependsOn;
    } else if (customModules[index].output_space == undefined) {
      if (customModules.length > 1) {
        spaceItDependsOn = customModules[index + 1].input_space;
      }
  ...
  addNewModuleToUse(
    ...
    if (spaceItDependsOn == undefined) {
      ...
      let response = this.addCustomModuleToList(
        customModules,
        modules,
        moduleName,
        key,
      ...
      currentModuleList.splice(
        index,
        0,
        this.setUseModuleFromCustomModule(newCustomModule)
      );
    }
  }
  ...
  removeModuleToUse(
    ...
    return this.defineCustomModuleList(
      currentModuleList,
      customModules,
      modules,
      key,
