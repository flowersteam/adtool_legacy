...
export class ArchiveExperimentComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.archiveForm.valueChanges.subscribe((formControls) => {
      this.allowDeleteButton = Object.values(formControls).some(
        (element) => element
      );
    });
  }
  ...
  ngAfterViewInit(): void {
    var tooltipsTriggerList = [].slice.call(
      document.querySelectorAll('#removeModal label[data-toggle="tooltip"]')
    );
  ...
  updateForm(): void {
    if (this.experiment) {
      this.archiveForm.controls['archiveExperiment'].valueChanges.subscribe(
        (value) => {
          if (value) {
            this.archiveForm.controls['archiveChekpointSaves'].setValue(true);
            this.archiveForm.controls['archiveChekpointSaves'].disable({
              emitEvent: false,
            });
            this.archiveForm.controls['archiveDiscoveries'].setValue(true);
            this.archiveForm.controls['archiveDiscoveries'].disable({
              emitEvent: false,
            });
          } else {
            this.archiveForm.controls['archiveChekpointSaves'].enable({
              emitEvent: false,
            });
            this.archiveForm.controls['archiveChekpointSaves'].setValue(
              this.experiment?.checkpoint_saves_archived
            );
            this.archiveForm.controls['archiveDiscoveries'].enable({
              emitEvent: false,
            });
            this.archiveForm.controls['archiveDiscoveries'].setValue(
              this.experiment?.discoveries_archived
            );
          }
        }
      );

      this.archiveForm.controls['archiveChekpointSaves'].valueChanges.subscribe(
        (value) => {
      ...
      this.archiveForm.controls['archiveDiscoveries'].valueChanges.subscribe(
        (value) => {
      ...
      this.archiveForm.controls['archiveExperiment'].setValue(
        this.experiment.archived
      );
      this.archiveForm.controls['archiveDiscoveries'].setValue(
        this.experiment.discoveries_archived
      );
  ...
  applyRemoval(): void {
    if (this.experiment) {
      var archiveCheckpointSavesValue = this.archiveForm.controls[
        'archiveChekpointSaves'
      ].value as boolean;
      if (
        archiveCheckpointSavesValue &&
        archiveCheckpointSavesValue != this.experiment.checkpoint_saves_archived
      ) {
        this.toasterService.showInfo('Removing checkpoint saves', 'Remove');
        this.experiment.checkpoints.forEach((checkpoint) => {
          this.expeDBService
            .deleteCheckpointSaves(checkpoint.id)
            .subscribe((response) => {
              if (response.success) {
                console.log(
                  'Removed checkpoint save for checkpoint n°' + checkpoint.id
                );
              } else {
                this.toasterService.showError(
                  response.message ?? '',
                  'Error removing checkpoint n°' + checkpoint.id
                );
              }
            });
        });
        this.allowDeleteModal = false;
        this.appDBService
          .archiveExperimentCheckpointSavesById(this.experiment.id)
          .subscribe((response) => {
            if (!response.success) {
              this.toasterService.showError(
                response.message ?? '',
                'Error archiving checkpoint saves in AppDB'
              );
            }
      ...
      var archiveDiscoveriesValue = this.archiveForm.controls[
        'archiveDiscoveries'
      ].value as boolean;
      if (
        archiveDiscoveriesValue &&
        archiveDiscoveriesValue != this.experiment.discoveries_archived
      ) {
        this.toasterService.showInfo('Removing discoveries', 'Remove');
        this.experiment.checkpoints.forEach((checkpoint) => {
          this.expeDBService
            .deleteCheckpointDiscoveries(checkpoint.id)
            .subscribe((response) => {
              if (response.success) {
                console.log(
                  'Removed discoveries for checkpoint n°' + checkpoint.id
                );
              } else {
                this.toasterService.showError(
                  response.message ?? '',
                  'Error removing discoveries of checkpoint n°' + checkpoint.id
                );
              }
            });
        });
        this.allowDeleteModal = false;
        this.appDBService
          .archiveExperimentDiscoveriesById(this.experiment.id)
          .subscribe((response) => {
            if (!response.success) {
              this.toasterService.showError(
                response.message ?? '',
                'Error archiving discoveries in AppDB'
              );
            }
      ...
      var archiveExperimentValue = this.archiveForm.controls[
        'archiveExperiment'
      ].value as boolean;
      if (archiveExperimentValue != this.experiment.archived) {
        ...
        this.appDBService
          .updateArchiveExperimentStatusById(
            this.experiment.id,
            archiveExperimentValue
          )
          .subscribe((response) => {
            if (!response.success) {
              this.toasterService.showError(
                response.message ?? '',
                'Error archiving experiment in AppDB'
              );
            }
