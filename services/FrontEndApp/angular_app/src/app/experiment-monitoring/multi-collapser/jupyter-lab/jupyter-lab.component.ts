...
export class JupyterLabComponent implements OnInit {
  ...
  constructor(public sanitizer: DomSanitizer, private jupyterService: JupyterService) {}
  ...
  defineJupyterLab(){
    if(!this.kernelInfoSet && this.experiment){
      this.defineInfoToAccessJupyterLab(this.experiment.name, this.experiment.id);
    }
  }
