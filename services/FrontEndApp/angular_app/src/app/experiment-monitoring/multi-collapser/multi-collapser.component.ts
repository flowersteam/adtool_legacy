...
export class MultiCollapserComponent implements OnInit {
  ...
  collapseVisualisation(event: any) {
    var collapseTriggerList: any = [].slice.call(
      document.querySelectorAll(this.fromArrayNameToQueryString())
    );
    var collapseBtnTriggeringList = [];
    for (let collapseTrigger of collapseTriggerList) {
      if (
        event.delegateTarget.id.replace('btn', '') == collapseTrigger.id ||
        collapseTrigger.classList.value.includes('show')
      ) {
        collapseBtnTriggeringList.push(collapseTrigger);
      }
    }

    var collapseList = collapseBtnTriggeringList.map(function (
      collapseTriggerEl
    ) {
