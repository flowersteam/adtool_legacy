...
export class LoadExperimentConfigToCreateComponent implements OnInit {
  ...
  openDialog(): void {
    const dialogRef = this.dialog.open(ChoosePreviousExperimentComponent, {
      data: { id: this.previous_experiment_id },
    });
  ...
  downloadJson() {
    ...
    element.setAttribute(
      'download',
      this.currentExperiment.experiment.name + '.json'
    );
  ...
  setExperimentWithJSON() {
    ...
    this.currentExperiment.output_representations =
      loadedExperiment.output_representations;
  ...
  setExperimentWithPreviousExperiment(id: number) {
    this.appDBService.getExperimentById(id).subscribe((response) => {
      if (response.success && response.data) {
        ...
        this.currentExperiment.output_representations =
          experiment.output_representations;
        this.currentExperiment.system = experiment.systems[0];
      } else {
        this.toasterService.showError(
          response.message ?? '',
          'Error getting previous experiment'
        );
      }
